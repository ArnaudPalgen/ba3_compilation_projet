...
class Node:
    ...
    """
    Noeud d'un arbre syntaxique/semantique.
    Un noeud est un arbre
    """

    def __init__(self, p_type, children=[], value=None, function=lambda node: node.value):
        """
        Constructeur
        :param p_type: le type du noeur
        :param children: les enfants du noeud. Par defaut: une liste vide
        :param value: la valeur du noeud. Par defaut: None
        :param function: la fonction d'evaluation du noeud. Par defaut: une fonction qui retourne la valeur du noeud
        """
        self.p_type = p_type
    ...
    def eval(self):
        """
        :return: Le resultat de la function d'evaluation
        """
        return self.function(self)


error = False  # True si l'analyse syntaxique a rencontre une erreur, False sinon
...
def p_expression_integer_expression(p):
    """integer_expression : integer_expression ADD_OP integer_expression
        ...
        | integer"""
    if len(p) == 4:
        p[0] = Node("integer_expression", [p[1], Node("op", value=p[2]), p[3]])
    elif len(p) == 2:
...
parser = yacc.yacc(outputdir="production")


def analyse(file):
    """
    Lance l'analyse syntaxique
    :param: Le fichier a analyser
    :return: L'arbre d'analyse syntaxique
    """
    f = open(file).read()
